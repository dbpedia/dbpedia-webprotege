...
public class AuthenticatedActionExecutor {
    ...
    Logger logger = Logger.getLogger(AuthenticatedActionExecutor.class.getName());
        ...
        dispatchServiceManager.execute(new GetChapSessionAction(userId), new DispatchServiceCallback<GetChapSessionResult>() {
            ...
            public void handleSuccess(GetChapSessionResult result) {
                ...
                if (chapSession.isPresent()) {
                    logger.info("[AutenticatedActionExecutor] chapSession is present.");
                    constructAndSendResponse(userId, clearTextPassword, chapSession.get(), actionFactory, callback);
                } else {
                    if(result.unknown_user.get()) {
                        logger.info("[AutenticatedActionExecutor] UNKNOWN USER");
                        callback.onSuccess(AuthenticationResponse.UNKNOWN_USER);
                    } else {
                        logger.info("[AutenticatedActionExecutor] chapSession is not present.");
